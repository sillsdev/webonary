...
class InfoTest extends WP_UnitTestCase
{
	function test_Import_Status()
	{
		...
		$status = Webonary_Info::import_status();
		$stripped = strip_tags($status);

		$this->assertContains('1 entries imported', $stripped);
		$this->assertContains('Importing...', $status);

		$this->assertContains('id="sil-count-imported"', $status);

		update_option('importStatus', 'indexing');
		$status = Webonary_Info::import_status();

		$this->assertContains('id="btnReindex"', $status);
	...
	function test_postCountByImportStatus($assert = true)
	{
		...
		$post_count = Webonary_Info::postCountByImportStatus();

		if($assert)
		{
			$this->assertNotEmpty($post_count);
			$this->assertEquals(2, $post_count->total_count);
			$this->assertEquals(0, $post_count->indexed_count);
		}
	}
