...
Complete list of API calls are found in [Webonary Cloud API Guide](./API.md).
## Architecture
...
2. Directory structure:
   ...
   4. Tools and utlities are in `tools`. 
   5. Unit tests are stored in `__tests__` within the directory where the code to be tested is. 
   6. Unit code test coverage informnation goes to `coverage`.
...
2. [Visual Studio Code (VSC)](https://code.visualstudio.com/) can be (configured)[https://levelup.gitconnected.com/setting-up-eslint-with-prettier-typescript-and-visual-studio-code-d113bbec9857] so automatic changes by eslint and prettier can be done as you save your code. 
   1. Altenrately, you can run `npm run lint` from the root directory to detect problems and make automatic changes in all your Typescript files.
...
5. If you make changes to you cdk stack code, or to any of your Lambda fucntions, you can deploy your changes by doing `npm run deploy`.
...
2. Make sure `.env` file in your root directory is set up with the Webonary username and password for that dictionary. WCA uses Webonary's Wordpress username and password for authoriztion (using http basic authentication).
3. To load the entire dictionary, run `npm run load-legacy dicitionary_name configured.xhtml` where dictionary_name is the dictionary site name in Webonary.
   1. To limit the number of entries loaded, you can pass in a number as the final argument. For example, to load only the first 10 items, do `npm run load-legacy dicitionary_name configured.xhtml 10`.
   2. Note that the load-legacy script was written to accomodate only a few model dictionaries, such as 
