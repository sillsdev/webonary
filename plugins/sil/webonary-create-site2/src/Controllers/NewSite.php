...
class NewSite
{
	public static function DisplayNewSite(string $app_id = null): string
	{
		if (!empty($app_id) && $app_id != 'new') {
			...
			$sub_title = sprintf(__('Language: %s', Main::$text_domain), $app->LanguageName);
		}
		else {
			$app = new Application();
			$sub_title = '';
		...
		$button_text = __('Create Now', Main::$text_domain);
...
HTML;
	...
	{
		$info = self::BuildInfoBlock($field['info'] ?? '');
		$field_label = __($field['label'], Main::$text_domain);
...
HTML;
	...
	public static function AjaxCreateSite(): array
	{
		list($messages, $from_blog_id, $domain, $title) = self::CheckPostData();

		if (!empty($messages))
			return self::AjaxReturn(['errors' => $messages]);

		// get the admin user
		list($user_id, $password) = self::GetSiteAdmin();

		$result = self::CreateNewBlog($user_id, $domain, $title);
		$success_msg = self::CopyTemplateToBlog($from_blog_id, $result['blog_id'], $user_id, $password);

		$app_id = $_POST['app-id'];
		if ($app_id != 'new')
			Applications::GetByID($app_id)->MarkCreated();

		return self::AjaxReturn(['status' => 'OK', 'msg' => $success_msg]);
	}

	private static function AjaxReturn(array $return_val): ?array
	{
		if (defined('PHP_UNIT'))
			return $return_val;
	...
	private static function CheckPostData(): array
	{
		...
		if (!$from_blog_id)
			$messages[] = __('Please select a Source Site to Copy.', Main::$text_domain);

		$domain = sanitize_user(str_replace('/', '', $_POST['desired-url'] ?? ''));
		if (empty($domain))
			$messages[] = __('Please enter a New Site Address.', Main::$text_domain);

		$title = $_POST['language-name'] ?? '';
		if (empty($title))
			$messages[] = __('Please enter a New Site Title.', Main::$text_domain);

		return [$messages, $from_blog_id, $domain, $title];
	}

	public static function GetSiteAdmin(): array
	{
		/** @var $wpdb wpdb */
		global $wpdb;
		...
		if (!empty($row->ID)) {
			...
			return self::AjaxReturn(['errors' => [$error]]);
		}
	...
	public static function CreateNewBlog(int $owner_user_id, string $path, string $title): array
	{
		// we only support sub-directory domains, not subdomains
...
TXT;
		...
		$sql = $wpdb->prepare("UPDATE {$prefix}postmeta SET meta_value = %s WHERE meta_key = '_mail'", maybe_serialize($contact_options));
		$wpdb->query($sql);
		...
		// sprintf(__('Copied: %s in %s seconds', Main::$text_domain), '<a href="' . $site_url . '" target="_blank">' . $to_blog_info->blogname . '</a>', number_format_i18n(timer_stop())),
		$msg = [
...
HTML;
		...
		$line1 = sprintf(__('Copied: %s in %s seconds', Main::$text_domain), '<a href="' . $site_url . '" target="_blank">' . $to_blog_info->blogname . '</a>', number_format_i18n(timer_stop()));
...
HTML;
		...
		do_action('log', __('Copy Complete!', Main::$text_domain), Main::$text_domain, $msg);
		do_action('copy_blog_complete', $from_blog_id, $to_blog_id);

		return $msg;
	}

	public static function CopyBlogData(int $from_blog_id, int $to_blog_id): void
	{
		/** @var $wpdb wpdb */
		global $wpdb;
		...
		do_action('log', $query, Main::$text_domain);
		...
		foreach ($saved_options as $option_name => $option_value) {
			...
			if ($option_name == 'admin_email') {
				$sql = "UPDATE {$prefix}options SET option_value = %s WHERE option_name = 'admin_email'";
		...
		do_action('log', $query, Main::$text_domain, count($options) . ' results found.');
		if ($options) {
